...
export default class extends LayerProvider {
  ...
  constructor(manager) {
    super(manager);
    this.name = 'Bestemmingsplangebied';
  ...
  render() {
    this.visible = true;
    this.layer = L.tileLayer.wms('https://geodata.nationaalgeoregister.nl/plu/wms', {
      layers: 'Bestemmingsplangebied',
      format: 'image/png',
      transparent: true,
      styles: 'plu:Bestemmingsplangebied'
    });
    return this.layer;
  }
    ...
    return new Promise(resolve => {
      axios.get(url)
        .then(({data}) => {
          ...
          if (data.features) {
            resolve({
              label: 'Bestemmingsplangebied',
              source: this.name,
              layer: this.itemGeoJsonLayer,
              data: data.features.map((feature) => {
                feature.properties.formattedProps = this.formatResponse(feature.properties)
  ...
  formatResponse(response) {
    return _.values(_.mapValues(response, function(value, key){
      if (typeof value === 'string' && value.startsWith('http://')) {
        return {title: key, value: value.split(','), type: 'link'}
      }
      return {title: key, value: value, type: 'text'}
      }));
  }
